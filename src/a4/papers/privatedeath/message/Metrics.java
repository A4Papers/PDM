...
public class Metrics {
	...
	public Metrics(Plugin plugin) throws IOException {
		...
		this.configuration.addDefault("Metrics.optout", Boolean.valueOf(false));
		this.configuration.addDefault("Metrics.guid", UUID.randomUUID()
				.toString());
		this.configuration.addDefault("Metrics.debug", Boolean.valueOf(false));
		if (this.configuration.get("Metrics.guid", null) == null) {
			this.configuration
					.options()
					.header("PrivateDeath config file - To disable auto updating go to 'Update.yml'\nFor more info go to 'http://goo.gl/DDUjlF'")
					.copyDefaults(true);
			this.configuration.save(this.configurationFile);
		}
		this.guid = this.configuration.getString("Metrics.guid");
		this.debug = this.configuration.getBoolean("Metrics.debug", false);
	}
	...
	public boolean isOptOut() {
		synchronized (this.optOutLock) {
			...
			return this.configuration.getBoolean("Metrics.optout", false);
		}
	}
	...
	public void enable() throws IOException {
		synchronized (this.optOutLock) {
			if (isOptOut()) {
				this.configuration
						.set("Metrics.optout", Boolean.valueOf(false));
				this.configuration.save(this.configurationFile);
			}
			if (this.task == null) {
				start();
	...
	public void disable() throws IOException {
		synchronized (this.optOutLock) {
			if (!isOptOut()) {
				this.configuration.set("Metrics.optout", Boolean.valueOf(true));
				this.configuration.save(this.configurationFile);
			}
			if (this.task != null) {
				this.task.cancel();
	...
	public File getConfigFile() {
		...
		return new File(new File(pluginsFolder, "PrivateDeathMessage"),
				"config.yml");
	}
